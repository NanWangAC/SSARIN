...
class HyperNet(nn.Module):
    def __init__(self, K, HSI_Num):
        ...
        self.conv1 = nn.Sequential(
            nn.Conv2d(in_channels=HSI_Num, out_channels=K*128, kernel_size=(3, 3), stride=(1, 1), padding=(1,1)),
            nn.BatchNorm2d(num_features=K*128),
            nn.ReLU(inplace=True))
        self.conv2 = nn.Sequential(
            nn.Conv2d(in_channels=K*128, out_channels=K*64, kernel_size=(3, 3), stride=(1, 1), padding=(1,1)),
            nn.BatchNorm2d(num_features=K*64),
            nn.ReLU(inplace=True))

        self.conv4 = nn.Sequential(
            nn.Conv2d(in_channels =K*64, out_channels=K*128, kernel_size=(1, 1)),
            nn.BatchNorm2d(num_features=K*128),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels =K*128, out_channels=K*256, kernel_size=(3, 3),padding=(1,1)),
            nn.BatchNorm2d(num_features=K*256),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels =K*256, out_channels=K*128, kernel_size=(5, 5),padding=(1,1)),
            nn.BatchNorm2d(num_features=K*128),
            nn.ReLU(inplace=True),
            nn.Conv2d(in_channels =K*128, out_channels=128, kernel_size=(3, 3),padding=(1,1)),
